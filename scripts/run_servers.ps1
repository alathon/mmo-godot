...
$procs = @(
    Start-Process cmd -ArgumentList "/k", "title Orchestrator && cd /d `"$root`" && godot --headless --scene res://src/orchestrator/Orchestrator.tscn" -PassThru
    Start-Process cmd -ArgumentList "/k", "title Zone: forest (9002) && cd /d `"$root`" && godot --headless --scene `"res://src/game-server/zones/ServerZone.tscn`" -- $forestArgs" -PassThru
    Start-Process cmd -ArgumentList "/k", "title Zone: other (9003) && cd /d `"$root`" && godot --headless --scene `"res://src/game-server/zones/ServerZone.tscn`" -- $otherArgs" -PassThru
)
