...
## Main per-tick entry point. Called by ServerZone after player simulation.
##
## zone_players:   entity_id -> CommonPlayer node (each has MobStats / MobCooldowns / MobCombatState children)
## ability_inputs: entity_id -> {ability_id, target_entity_id, ground_x/y/z}
## moving_entities: entity_id -> true
##
## Returns Array of per-peer ACK packets: [{peer_id: int, bytes: PackedByteArray}]
## The caller is responsible for broadcasting WorldState (vitals + combat events)
## via build_combat_events_proto().
func tick(sim_tick: int, zone_players: Dictionary,
		ability_inputs: Dictionary, moving_entities: Dictionary) -> Array:
	_pending_events.clear()
	_ack_queue.clear()

	# 1. Cancel casts for entities that moved
	for entity_id in moving_entities:
		var combat := _combat(entity_id, zone_players)
		var cds    := _cooldowns(entity_id, zone_players)
	...
	for entity_id in ability_inputs:
		var combat := _combat(entity_id, zone_players)
		var cds    := _cooldowns(entity_id, zone_players)
		var stats  := _stats(entity_id, zone_players)
		if combat == null or cds == null or stats == null:
			continue
		_process_ability_input(entity_id, combat, cds, stats,
				ability_inputs[entity_id], sim_tick, zone_players)

	# 3. Advance timers and resolve completed casts
	for entity_id in zone_players:
		var combat := _combat(entity_id, zone_players)
		var cds    := _cooldowns(entity_id, zone_players)
		var stats  := _stats(entity_id, zone_players)
		if combat == null or cds == null or stats == null:
			continue
		_advance_and_resolve(entity_id, combat, cds, stats, sim_tick, zone_players)
