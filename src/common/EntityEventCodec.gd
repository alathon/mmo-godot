...
static func write_event(msg, event: EntityEvents, sim_tick: int) -> void:
	...
	match event.type:
		EntityEvents.Type.ABILITY_USE_STARTED:
			...
			started.set_ability_id(String(event.ability_id))
		...
		EntityEvents.Type.ABILITY_USE_CANCELED:
			...
			canceled.set_ability_id(String(event.ability_id))
		...
		EntityEvents.Type.ABILITY_USE_COMPLETED:
			...
			completed.set_ability_id(String(event.ability_id))
		...
		EntityEvents.Type.DAMAGE_TAKEN:
			...
			damage.set_ability_id(String(event.ability_id))
		...
		EntityEvents.Type.BUFF_APPLIED:
			...
			buff.set_ability_id(String(event.ability_id))
			buff.set_status_id(String(event.status_effect_id))
			buff.set_stacks(1)
		...
		EntityEvents.Type.DEBUFF_APPLIED:
			...
			debuff.set_ability_id(String(event.ability_id))
			debuff.set_status_id(String(event.status_effect_id))
			debuff.set_stacks(1)
		...
		EntityEvents.Type.STATUS_EFFECT_REMOVED:
			...
			removed.set_status_id(String(event.status_effect_id))
